    ...
    if($mybb->input['username'] || $mybb->input['guest'])
    {
        ...
        */
        ...
        foreach($userpermissions as $key => $value)
        {
            ...
            {
                $my_string = $language_strings[$key];
                $language_string = $lang->$my_string;
                if(!$language_string)
                {
                    $language_string = $my_string;
                }
            }
            else
            {
            ...
            if(!in_array($key, $groupzerogreater))
            {
                ...
                else
                {
                    ...
                    $exgroups = explode(",", $value);
                    $cellcontent = "";
                    foreach ($exgroups as $gid)
                    {
                        $cellcontent .= $cellcomma . $usergroups[$gid]['title'];
                        $cellcomma = ", ";
                    }
                    $table->construct_cell($cellcontent);
                }
            }
            else
            {
    ...
    if($mybb->input['username'] || $mybb->input['guest'] == 1)
    {
        ...
        foreach($forums as $forum)
        {
            ...
            foreach($forumpermissions as $permissionname => $value)
            {
                ...
                $forumkey = $good_keys[$permissionname];
                $table->construct_cell($lang->$forumkey);
                if($value == 1)
                {
                    $table->construct_cell($lang->yes);
                }
